...
void NameSpaceService::CreateFile(
  ...
  retCode = kCurveFS.CreateFile(request->filename(), request->poolset(),
                                request->owner(), request->filetype(),
                                request->filelength(), request->stripeunit(),
                                request->stripecount());
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
...
void NameSpaceService::CreateCloneFile(
  ...
  // create clone file
  ret = kCurveFS.CreateCloneFile(
      request->filename(), request->owner(), request->filetype(),
      request->filelength(), request->seq(), request->chunksize(),
      request->stripeunit(), request->stripecount(), request->poolset(),
      response->mutable_fileinfo(), request->clonesource(),
      request->filelength());
  response->set_statuscode(ret);
  if (ret != StatusCode::kOK) {
    if (google::ERROR != GetMdsLogLevel(ret)) {
