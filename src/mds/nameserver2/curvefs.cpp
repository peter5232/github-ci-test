...
StatusCode CurveFS::CreateFile(const std::string &fileName, std::string poolset,
  ...
  if (storeRet == StoreStatus::OK) {
    ...
    if (allocateIfNoExist == false) {
      ...
      auto ifok = chunkSegAllocator_->AllocateChunkSegment(
          ...
          fileInfo.has_poolset() ? fileInfo.poolset() : kDefaultPoolsetName,
          offset, segment);
      if (ifok == false) {
...
StatusCode CurveFS::CreateCloneFile(const std::string &fileName,
                                    ...
                                    std::string poolset, FileInfo *retFileInfo,
...
uint32_t CurveFS::GetBlockSize() const { return g_block_size; }
